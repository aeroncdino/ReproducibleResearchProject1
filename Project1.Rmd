...
par(mfrow = c(1, 2))
       hist(totalStepsDay$steps, breaks=20, xlab="Number of Steps Taken", family="serif", ylim=c(0,20), main="")
       
       hist(totalStepsDayImputed$steps_imputed, breaks=20, xlab="Number of Steps Taken",family="serif", ylim=c(0,20), main="")
...
activity_Weekend <- activity %>%
  mutate( is_Weekened = ifelse(weekdays(date) %in% c("Saturday", "Sunday"), "weekend", "weekday"))
 
library(ggplot2)
library(dplyr)

avg_steps <- activity_Weekend %>%
  group_by(interval, is_Weekened) %>%
  summarise(mean_steps = mean(steps, na.rm = TRUE))
ggplot(avg_steps, aes(x = interval, y = mean_steps)) +
  geom_line() +
  facet_wrap(~is_Weekened, ncol = 1) +
